...
export default function ContactsPage() {
  ...
  const [contactEditForm, setContactEditForm] = useState<Partial<Contact>>({})
  ...
  const startEditingContact = () => {
    if (selectedContact) {
      setContactEditForm({ ...selectedContact })
  ...
  const saveContactEdit = () => {
    if (!selectedContact) return
    updateContact(selectedContact.id, contactEditForm)
    setIsEditingContact(false)
    toast.success("Contact mis à jour")
  }
  ...
  if (!isContactsConnected) {
    return (
      <div className="relative flex h-full w-full items-center justify-center bg-gradient-to-br from-slate-50 via-emerald-50/30 to-green-50/40 p-6 overflow-auto">
        <div className="pointer-events-none absolute inset-0 overflow-hidden">
          <div className="absolute -top-32 -left-32 w-96 h-96 bg-emerald-100/40 rounded-full blur-3xl" />
          <div className="absolute -bottom-32 -right-32 w-96 h-96 bg-green-100/40 rounded-full blur-3xl" />
        </div>
        ...
        >
          <div className="flex justify-center">
            <div className="relative">
              <div className="w-28 h-28 rounded-3xl bg-gradient-to-br from-emerald-500 to-green-600 flex items-center justify-center shadow-2xl shadow-emerald-500/30">
                <Users className="w-14 h-14 text-white" />
              </div>
              <motion.div
                animate={{ scale: [1, 1.15, 1] }}
                transition={{ duration: 2, repeat: Infinity }}
                className="absolute -top-2 -right-2 w-9 h-9 bg-red-500 rounded-full flex items-center justify-center shadow-lg"
              >
          ...
          <div className="space-y-3">
            ...
            <p className="text-gray-500 text-base sm:text-lg leading-relaxed max-w-lg mx-auto">
              Connectez Google Contacts pour retrouver, enrichir et interagir avec vos contacts grâce à l'IA de Bouba — en temps réel.
            </p>
          </div>
            ...
            {[
              {
                ...
                title: 'IA sur vos contacts',
                desc: '"Bouba, qui dois-je rappeler cette semaine ?"',
              },
              {
                icon: RefreshCw,
                color: 'bg-emerald-100 text-emerald-600',
                title: 'Sync Google Contacts',
                desc: "Carnet d'adresses toujours à jour",
              },
              {
                icon: Mail,
                color: 'bg-blue-100 text-blue-600',
                title: 'Email intégré',
                desc: 'Envoyez un email directement depuis le profil',
              },
          ...
          <div className="flex flex-col items-center gap-3">
            <button
              ...
              className="inline-flex items-center gap-2.5 bg-gradient-to-r from-emerald-600 to-green-600 hover:from-emerald-700 hover:to-green-700 text-white font-bold px-8 py-4 rounded-2xl text-base shadow-xl shadow-emerald-500/25 transition-all hover:scale-105 active:scale-100"
            >
      ...
      <div className="hidden md:flex w-60 lg:w-64 border-r border-border bg-surface/50 p-4 lg:p-6 flex-col gap-6 lg:gap-8 overflow-y-auto">
        <div className="space-y-3">
          ...
          <div className="rounded-2xl border border-primary/15 overflow-hidden">
            <div className="flex items-center gap-1.5 px-3 py-2 bg-gradient-to-r from-primary/8 to-violet-500/5 border-b border-primary/10">
              <Sparkles className="w-3 h-3 text-primary" />
              <span className="text-[9px] font-bold uppercase tracking-widest text-primary">Ajout via Bouba</span>
            </div>
            <div className="flex items-center bg-background/60">
              <input
                ...
                placeholder='Ex: "Alice Martin, alice@corp.com"'
                className="flex-1 min-w-0 bg-transparent px-3 py-2 text-[11px] outline-none placeholder:text-muted"
              />
              <button
                onClick={handleAiCommand}
                disabled={isProcessing || !aiCommand.trim()}
                className="m-1 p-1.5 bg-primary text-white rounded-lg hover:bg-primary-dark disabled:opacity-40 transition-colors shrink-0"
              >
                {isProcessing
