...
router.post('/payments', auth_1.authenticate, async (req, res) => {
    try {
        ...
        // Insérer le paiement
        const payment = await (0, db_1.queryOne)(`
...
router.get('/:table', async (req, res) => {
    try {
        ...
        if (req.query.order) {
            const orderStr = req.query.order;
...
router.post('/:table', async (req, res) => {
    try {
        ...
        const placeholders = dataWithUserId.map((_, rowIndex) => `(${fields.map((_, fieldIndex) => `$${rowIndex * fields.length + fieldIndex + 1}`).join(', ')})`).join(', ');
        const sql = `
...
router.get('/plans', async (req, res) => {
    try {
        const plans = await (0, db_1.query)('SELECT * FROM public.plans WHERE active = true ORDER BY price ASC');
        res.json({
            success: true,
            data: plans
        });
