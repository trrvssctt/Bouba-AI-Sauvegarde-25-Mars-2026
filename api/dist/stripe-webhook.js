...
async function handleCheckoutCompleted(session) {
    ...
    if (meta.user_id && !email) {
        ...
        await (0, db_1.query)('UPDATE public.profiles SET subscription_status = $1, plan_id = $2 WHERE id = $3', ['active', plan_id || 'pro', userId]);
        // Upsert subscription
        if (session.subscription) {
         ...
         DO UPDATE SET status = $4, current_period_start = $5, current_period_end = $6, updated_at = NOW()`, [userId, plan_id, sub.id, 'active',
    ...
    const existing = await (0, db_1.queryOne)('SELECT id FROM public.users WHERE email = $1', [email]);
    if (existing) {
        console.log('User already exists, activating subscription:', existing.id);
        await (0, db_1.query)('UPDATE public.profiles SET subscription_status = $1 WHERE id = $2', ['active', existing.id]);
        return;
    }
    ...
    // Create user
    const newUser = await (0, db_1.queryOne)(`INSERT INTO public.users (email, name, provider, password_hash, email_verified, role_id)
     VALUES ($1, $2, $3, $4, $5, $6)
     RETURNING id`, [email, fullName || null, 'email', password_hash, false, defaultRole?.id || null]);
    if (!newUser) {
    ...
    (0, email_1.sendPaymentConfirmationEmail)(email, first_name, plan_id || 'pro', session.amount_total || 0, session.currency || 'eur')
...
async function handleSubscriptionChange(subscription) {
    ...
    const oldSub = await (0, db_1.queryOne)('SELECT plan_id FROM public.subscriptions WHERE stripe_subscription_id = $1', [subscription.id]);
    const oldPlanId = oldSub?.plan_id;
    // Update subscription in database
    await (0, db_1.query)(`INSERT INTO public.subscriptions (user_id, plan_id, stripe_subscription_id, status, current_period_start, current_period_end, cancel_at_period_end)
    ...
    if (oldPlanId && plan_id && oldPlanId !== plan_id && status === 'active') {
        const user = await (0, db_1.queryOne)(`SELECT u.email, p.first_name FROM public.users u
       JOIN public.profiles p ON p.id = u.id
       WHERE u.id = $1`, [user_id]);
        if (user) {
            (0, email_1.sendPlanChangeEmail)(user.email, user.first_name, oldPlanId, plan_id)
...
async function handleSubscriptionDeleted(subscription) {
    ...
    await (0, db_1.query)(`INSERT INTO public.subscriptions (user_id, plan_id, stripe_subscription_id, status)
     VALUES ($1, $2, $3, $4)
     ON CONFLICT (stripe_subscription_id)
     DO UPDATE SET plan_id = $2, status = $4, updated_at = NOW()`, [user_id, 'starter', subscription.id, 'cancelled']);
...
async function handlePaymentFailed(invoice) {
    ...
    if (subscription) {
        ...
        if (user_id) {
            ...
            await (0, db_1.query)(`INSERT INTO public.payments (user_id, amount, currency, status, metadata)
         VALUES ($1, $2, $3, $4, $5)`, [
                user_id,
                invoice.amount_due,
                invoice.currency,
...
function mapStripeStatusToLocal(status) {
    switch (status) {
        case 'active':
            return 'active';
        case 'past_due':
