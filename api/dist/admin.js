...
exports.supportRouter = exports.notificationsRouter = void 0;
...
router.use(auth_1.authenticate, adminOnly);
// ─────────────────────────────────────────────
// USERS
// ─────────────────────────────────────────────
...
router.get('/users/:id', async (req, res) => {
    ...
    `, [id]);
      ...
      LIMIT 10
    `, [id]).catch(() => []);
        const billing = await (0, db_1.query)(`
      SELECT
        id,
        created_at AS date,
        amount,
      ...
      LIMIT 20
    `, [id]).catch(() => []);
        const connections = await (0, db_1.query)(`
    ...
    `, [id]).catch(() => []);
        res.json({
            ...
            data: {
                ...
                billing,
                connections: connections.map((c) => c.provider_name)
            }
        });
    }
    catch (err) {
...
router.put('/users/:id', async (req, res) => {
    ...
    `, [plan || null, status || null, suspensionReason || null, id]);
        ...
        if (status === 'suspended') {
            const targetUser = await (0, db_1.queryOne)(`
...
router.post('/users/:id/reset-quota', async (req, res) => {
    try {
        const { id } = req.params;
        await (0, db_1.query)(`UPDATE public.profiles SET messages_used = 0, updated_at = NOW() WHERE id = $1`, [id]);
        res.json({ success: true });
    }
    catch (err) {
        res.status(500).json({ success: false, error: 'Erreur serveur' });
...
router.get('/billing/stats', async (_req, res) => {
    try {
        const mrr = await (0, db_1.queryOne)(`
      SELECT
    ...
    `);
        const newMrr = await (0, db_1.queryOne)(`
      SELECT
    ...
    `);
        const churnMrr = await (0, db_1.queryOne)(`
...
router.put('/billing/upgrade-requests/:id', async (req, res) => {
    try {
        const { id } = req.params;
...
router.get('/monitoring/agent-stats', async (_req, res) => {
    ...
    `).catch(() => []);
        ...
        const totals = await (0, db_1.queryOne)(`
      SELECT
    ...
    `).catch(() => null);
        ...
        const total = raw.reduce((s, r) => s + (r.calls || 0), 0);
...
router.get('/support/tickets', async (_req, res) => {
    try {
        const tickets = await (0, db_1.query)(`
      SELECT
...
router.get('/settings/flags', async (_req, res) => {
    try {
        const flags = await (0, db_1.query)(`
...
router.delete('/settings/flags/:id', async (req, res) => {
    try {
        await (0, db_1.query)(`DELETE FROM public.feature_flags WHERE id = $1`, [req.params.id]);
        res.json({ success: true });
    }
    catch (err) {
        res.status(500).json({ success: false, error: 'Erreur serveur' });
...
router.get('/analytics/overview', async (_req, res) => {
    try {
        const regs = await (0, db_1.queryOne)(`
      SELECT
        COUNT(*)                                                               AS total,
      ...
      FROM public.users
    `);
        const stats = await (0, db_1.queryOne)(`
      SELECT
        COUNT(*)                                          AS total,
    ...
    `);
        const firstMsg = await (0, db_1.queryOne)(`
...
router.post('/notifications', async (req, res) => {
    try {
      ...
      `, [type, campaignId, senderId, subject || null, msgBody.trim()]);
            const countRow = await (0, db_1.queryOne)(`
      ...
      `, [campaignId]);
            ...
            // Individuel
            if (!userId)
                return res.status(400).json({ success: false, error: 'userId requis pour envoi individuel' });
            const notif = await (0, db_1.queryOne)(`
        INSERT INTO public.notifications (user_id, type, campaign_id, sender_id, subject, body)
        VALUES ($1, $2, $3, $4, $5, $6)
...
exports.supportRouter.use(auth_1.authenticate);
